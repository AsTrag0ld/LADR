...
public class Statistiques extends Activity implements RadioGroup.OnCheckedChangeListener {
    ...
    public void onCheckedChanged(RadioGroup group, int checkedId) {
        DatabaseHandler dbh = new DatabaseHandler(this);
        String [] from = {"nom", "nbVictoires"};
        int [] to = {R.id.test1, R.id.test2};

        if (group == this.group) {
            if (checkedId == R.id.radioBtnPoint) {
                String query = "SELECT rowid AS _id, nom, nbVictoires FROM Joueur j ORDER BY nbVictoires DESC";
                SQLiteDatabase db = dbh.getReadableDatabase();
                Cursor c = db.rawQuery(query, null);
                SimpleCursorAdapter adapter = new SimpleCursorAdapter(this, R.layout.row_test, c, from, to, 0);
                ListView lv = (ListView) findViewById(R.id.listStats);
                lv.setAdapter(adapter);
            } else if (checkedId == R.id.radioBtnVictoire) {
                String query = "SELECT rowid AS _id, nom, nbVictoires FROM Joueur j ORDER BY nbVictoires DESC";
